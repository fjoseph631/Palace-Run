...
public class EnemyInput : MonoBehaviour
{
    ...
    void detector()
    {
        ...
        if (GameManager.getManager().getDirection().Count > 0)
        {
            //Turn Tiger Left
            if (
                GameManager.getManager().getDirection().Peek() == GameManager.turnDirection.Left
            ...
            //Turn Tiger Right
            if (
                GameManager.getManager().getDirection().Peek() == GameManager.turnDirection.Right
                && GameManager.getManager().getCanSwipe()
            )
            {
                //transform.Rotate(0, 90, 0);
    ...
    void OnControllerColliderHit(ControllerColliderHit collision)
    {
        ...
        {
            anim.Play(Constants.AnimationHit, 0);
